...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // ============================================================
      // ViZPOS -> D1 MENU FULL SNAPSHOT SYNC
      // Auth: X-Sync-Key == env.SYNC_KEY
      // Stable identity: source_type + source_item_id
      // D1 id remains independent/autogenerated.
      // ============================================================
      if (url.pathname === "/api/sync/menu" && request.method === "POST") {
        ...
        if (!incoming.length) {
          return json({ ok: false, error: "items array is required and cannot be empty" }, 400);
        }

        const sourceType = String(body.source_type || incoming[0]?.source_type || "vizposfnb").trim();
        if (!sourceType) return json({ ok: false, error: "source_type is required" }, 400);
        ...
        for (const raw of incoming) {
          ...
          const sourceId = Number(item.source_item_id ?? item.itemid);
          const name = String(item.name_en ?? item.itemname ?? "").trim();
          ...
          if (!name) return json({ ok: false, error: `Empty menu item name for source_item_id ${sourceId}` }, 400);
          if (!Number.isFinite(dine) || !Number.isFinite(take)) {
            return json({ ok: false, error: `Invalid price for source_item_id ${sourceId}` }, 400);
          }
          ...
          const existing = await env.DB.prepare(`
            SELECT id, name_en, category, dine_in_price, takeaway_price, is_available
            FROM menu_items
          ...
          if (!existing) {
            ...
            `).bind(
              ...
              dine,
              take,
              forceUnavailable ? 0 : 1,
              sourceType,
              sourceId
            ).run();
            inserted++;
            if (forceUnavailable) disabled++;
            continue;
          }

          const availabilityNeedsChange = forceUnavailable && Number(existing.is_available) !== 0;
          const dataNeedsChange = String(existing.name_en ?? "") !== name
            || String(existing.category ?? "") !== category
            || Number(existing.dine_in_price ?? 0) !== dine
            || Number(existing.takeaway_price ?? 0) !== take;

          if (dataNeedsChange || availabilityNeedsChange) {
            if (forceUnavailable) {
              await env.DB.prepare(`
                UPDATE menu_items
                SET name_en = ?, category = ?, dine_in_price = ?, takeaway_price = ?, is_available = 0
                WHERE id = ?
              `).bind(name, category, dine, take, Number(existing.id)).run();
              disabled++;
            } else {
              await env.DB.prepare(`
                UPDATE menu_items
                SET name_en = ?, category = ?, dine_in_price = ?, takeaway_price = ?
                WHERE id = ?
              `).bind(name, category, dine, take, Number(existing.id)).run();
            }
            updated++;
        ...
        // Delete only rows previously managed by this sync source and absent
        // from the complete POS snapshot.
        const managedRows = await env.DB.prepare(`
          SELECT id, source_item_id, name_en
          FROM menu_items
          WHERE source_type = ? AND source_item_id IS NOT NULL
        `).bind(sourceType).all();

        for (const row of (managedRows.results || [])) {
          const key = `${sourceType}:${Number(row.source_item_id)}`;
          if (!seen.has(key)) {
            await env.DB.prepare(`DELETE FROM menu_items WHERE id = ?`).bind(Number(row.id)).run();
            deleted++;
          }
        }
