  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === "/api/auth/staff-login" && request.method === "POST") {
        ...
        const user=await env.DB.prepare(`SELECT * FROM profiles WHERE lower(email)=lower(?) AND password=? LIMIT 1`).bind(String(b.email||"").trim(),String(b.password||"")).first();
        if(!user)return json({ok:false,error:"Invalid email or password"},401);
        return json({ok:true,user:cleanRow(user),token:await staffToken(env,user)});
      }
